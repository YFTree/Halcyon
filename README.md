...
它以本地音乐播放为核心，同时集成了基础 i18n、本地歌单、WebDAV 远程曲库、LX Music API 在线音源、LRC / 增强 LRC / ELRC / TTML / Lyricify 歌词解析、动态播放页、沉浸式播放页、桌面歌词悬浮窗、Lyricon 集成、SuperLyricApi 歌词发布、Lyric Getter API 原文歌词传递、Flyme / AOSP 跑马灯歌词、媒体通知歌词、歌词卡片分享、AI 歌曲解读、FFmpeg 扩展解码、应用日志、备份恢复以及音乐库统计分析等能力。
...
- 音乐库提供独立搜索页，支持按歌曲、专辑、歌手筛选搜索结果，保存 / 删除搜索历史，并可一键查找同歌曲名、同专辑的重复歌曲。
...
- 小窗模式下播放页会隐藏迷你歌词，避免压缩底部播放控制区域；正常尺寸播放页仍保留迷你歌词显示。
...
- 支持 Flyme / AOSP 跑马灯歌词通知和媒体通知歌词；Flyme 状态栏歌词默认隐藏额外歌词通知，非 Flyme 设备可使用浮动通知歌词。
- 支持浮动通知歌词和媒体通知歌词副行内容配置，可在关闭、翻译、注音之间切换，并尽量保留逐词空格、尾部词和双行显示结构。
...
- 随机播放会保存原始队列顺序，退出随机播放后可恢复原列表顺序；从单曲循环切换到随机播放时优先原地重排队列以减少卡顿。
...
- 多选歌曲和单曲“添加到歌单”使用统一弹窗样式；歌曲信息条目支持长按复制，内置歌曲元数据编辑支持写回常用标签。
- MiniPlayer 封面会随播放而自动旋转，支持封面旋转开关、歌词显示开关、右侧按钮自定义、环形进度条、播放控制描边和歌曲 / 歌词切换动画。
...
| 系统歌词 | 桌面歌词、词幕、SuperLyricApi、Lyric Getter API、FLYme 状态栏歌词（Ticker 通知）、媒体通知歌词 |
