...
class DesktopLyricService : Service() {
    ...
    private fun addLyricView() {
        ...
        val root = LinearLayout(this).apply {
            ...
            addView(lyric, LinearLayout.LayoutParams(dp(460), dp(108)))
    ...
    private class DesktopLyricView(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            val primaryRight = agent.equals("v2", ignoreCase = true)
            val primaryX = if (hasBackground) {
                if (primaryRight) width * 0.72f else width * 0.28f
            } else {
                width / 2f
            }
            val backgroundX = if (primaryRight) width * 0.28f else width * 0.72f
            val baseline = height / 2f - if (translation.isBlank() && backgroundTranslation.isBlank()) -5f else 10f
            drawLine(canvas, text, words, primaryX, baseline, if (hasBackground) width * 0.42f else width * 0.9f, true)
            if (hasBackground) {
                drawLine(canvas, backgroundText.ifBlank { backgroundWords.joinToString("") { it.text } }, backgroundWords, backgroundX, baseline, width * 0.42f, false)
            }
            val translationText = listOf(translation, backgroundTranslation).firstOrNull { it.isNotBlank() }.orEmpty()
            if (translationText.isNotBlank()) drawFittedText(canvas, translationText, width / 2f, baseline + 25f * resources.displayMetrics.scaledDensity, width * 0.88f, translationPaint)
        }

        private fun drawLine(canvas: Canvas, fallbackText: String, lineWords: List<DesktopWord>, centerX: Float, baseline: Float, maxWidth: Float, primary: Boolean) {
            ...
            if (lineWords.isEmpty()) {
                drawFittedText(canvas, fallbackText, centerX, baseline, maxWidth, if (primary) activePaint else pendingPaint)
            } else {
                drawWords(canvas, centerX, baseline, maxWidth, lineWords, primary)
            }
        ...
        private fun drawWords(canvas: Canvas, centerX: Float, baseline: Float, maxWidth: Float, lineWords: List<DesktopWord>, primary: Boolean) {
            ...
            var x = centerX - ((scaledWidths.sum() + gap * (scaledWidths.size - 1).coerceAtLeast(0)) / 2f)
            displayWords.forEachIndexed { index, word ->
                ...
                if (isCurrent && word.endMs - word.startMs >= 900L) {
                    ...
                    canvas.drawText(token, x + scaledWidths[index] / 2f, baseline, glowPaint)
                }
                canvas.drawText(token, x + scaledWidths[index] / 2f, baseline, paint)
        ...
        private fun drawFittedText(canvas: Canvas, value: String, centerX: Float, baseline: Float, maxWidth: Float, paint: Paint) {
